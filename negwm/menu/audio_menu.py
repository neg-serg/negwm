...
class pulse_menu():
    ...
    def pulseaudio_output(self):
        ...
        for app in self.pulse_data["pulse_app_list"]:
            app_name = app.proplist["media.name"] + ' -- ' + app.proplist["application.name"]
    ...
    def pulseaudio_select_app(self, exclude_current_dev=True):
        if self.pulse is None:
            return 0
        menu_params = {
            'prompt': f'{menu.wrap_str("pulse app")} {self.prompt}',
        }
    ...
    def pulseaudio_select_output(self):
        ...
        if self.pulse is None:
            return None
        menu_params = {
            'prompt': f'{menu.wrap_str("pulse output")}{self.prompt}'
        }
...
def main():
    ...
    if len(sys.argv) > 1:
        match sys.argv[1]:
            case 'output': p.pulseaudio_output()
            case 'input': p.pulseaudio_input()
